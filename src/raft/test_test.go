...
func TestReElection2A(t *testing.T) {
	...
	log.Println("if the old leader rejoins, that shouldn't disturb the new leader.")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	log.Println("if there's no quorum, no leader should be elected.")
	...
	log.Println("if a quorum arises, it should elect a leader.")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	log.Println("re-join of last node shouldn't prevent leader from existing.")
	cfg.connect(leader2)
	cfg.checkOneLeader()
